...
class UI:
  ...
  def __addEntry(self, parent, name: str, properties: dict={}, placement: dict={}, pack: dict={}, validation: dict={}):
    """
    add an entry
    """
    ...
    if validation:
      ...
      if "type" in validation:
        ...
        if "integer" == validationType.lower():
          self.__UIElements[name].config(validate='key', validatecommand=(self.__validationInteger, '%S', '%P', maxLength))
        if "hour" == validationType.lower():
          self.__UIElements[name].config(validate='key', validatecommand=(self.__validationHour, '%S', '%P'))
        if "minute" == validationType.lower():
          self.__UIElements[name].config(validate='key', validatecommand=(self.__validationMinute, '%S', '%P'))
  ...
  def show(self):
    print("Show UI")
    if self.__baseName in self.__UIElements and self.__UIElements[self.__baseName]  != None:
